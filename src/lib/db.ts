...
export class DbManager {
  ...
  async getCache<T = unknown>(key: string): Promise<T | null> {
    const entry = DbManager.cache.get(key);
    if (!entry || entry.expiresAt <= Date.now()) {
      DbManager.cache.delete(key);
      return null;
    }
    return entry.value as T;
  }

  async setCache(key: string, value: unknown, expireSeconds: number): Promise<void> {
    DbManager.cache.set(key, { value, expiresAt: Date.now() + expireSeconds * 1000 });
  }

  async clearExpiredCache(prefix: string): Promise<void> {
    const now = Date.now();
    DbManager.cache.forEach((entry, key) => {
      if (key.startsWith(prefix) && entry.expiresAt <= now) DbManager.cache.delete(key);
    });
  }
